...
public class ItemEditor extends javax.swing.JPanel {
    ...
    private void saveItem() {
        ...
        try {
            ...
            if (item.getItemId() == 0) {
                CategoryExpandable ui = allCats.get(item.getCategoryId());
                if (ui == null) {
    ...
    private HashMap<Long, CategoryExpandable> allCats = new HashMap<>();

    private void insertInUI(Item item, ItemCategory category) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                CategoryExpandable catUI = allCats.get(category.getCategoryId());
    ...
    private CategoryExpandable createNewCategoryUI(ItemCategory category) {
        final CategoryExpandable lv_catUI = new CategoryExpandable();
