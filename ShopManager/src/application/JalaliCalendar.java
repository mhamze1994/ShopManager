...
public class JalaliCalendar {
    ...
    /**
     * get String with the following format : یکشنبه ۱۲ آبان
     *
    ...
    public static String format(long date) {
        try {
            ...
            String name = str.substring(0, 4) + "/" + str.substring(4, 6) + "/" + str.substring(6, 8);
            if (str.length() > 8) {
                name += " " + str.substring(8, 10) + ":" + str.substring(10, 12);
            }
            return name;
        } catch (Exception ex) {
            return "BAD FORMAT";
    ...
    public static long now() {
        return Long.parseLong(new JalaliCalendar().toString().replace("/", ""));
    }
